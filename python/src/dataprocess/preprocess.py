...
3. 使用双线性插值调整尺寸为 256x256 或 512x512
...
from src.dataprocess.config import DataProcessConfig
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[256, 512],
        default=256,
        help="目标分辨率：256 或 512 (默认：256)",
    )
    args = parser.parse_args()

    config = DataProcessConfig(image_size=args.size)

    danxia_dir = config.dataset_base / config.danxia_dir
    kasite_dir = config.dataset_base / config.kasite_dir

    output_base = config.output_base
    splits_dir = config.splits_base

    output_base.mkdir(exist_ok=True, parents=True)
    (output_base / "train").mkdir(exist_ok=True, parents=True)
    (output_base / "val").mkdir(exist_ok=True, parents=True)
    (output_base / "test").mkdir(exist_ok=True, parents=True)
    splits_dir.mkdir(exist_ok=True, parents=True)

    data = []

    print(f"正在加载 Danxia 图片 ({args.size}x{args.size})...")
    ...
    print(f"正在加载 Kasite 图片 ({args.size}x{args.size})...")
    ...
    for split_name, idx_list in split_info.items():
        ...
        for idx in idx_list:
            ...
            relative_path = f"{config.output_dir_name}/{split_name}/{new_filename}"
            csv_rows.append(
                {
