...
4. 输出 features.csv 或 features_512.csv
...
用法:
    ...
    # 512x512
    uv run python -m src.dataprocess.extract_features --size 512
"""
...
def main():
    ...
    normalized_dir = config.output_base
    ...
    for split in ["train", "val", "test"]:
        ...
        for img_path in sorted(split_dir.glob("*.png")):
            all_images.append(
                {
                    "path": str(img_path),
                    "filename": f"{config.output_dir_name}/{split}/{img_path.name}",
                    "type": "Danxia" if "danxia" in img_path.name.lower() else "Kasite",
                }
            )

    print(f"共找到 {len(all_images)} 张图片 ({args.size}x{args.size})")
    ...
    output_path = config.get_features_path()
    df.to_csv(output_path, index=False)

    print(f"\n✅ {args.size}x{args.size} 特征提取完成！")
