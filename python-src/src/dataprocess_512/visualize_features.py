...
try:
    ...
    print("⚠️ umap-learn not installed, skipping UMAP visualization")
...
def plot_tsne(df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.title("t-SNE Visualization (512x512, S/R/C Features)", fontsize=14)
    plt.legend()
...
def plot_umap(df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.title("UMAP Visualization (512x512, S/R/C Features)", fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_feature_distributions(df: pd.DataFrame, output_path: Path) -> None:
    ...
    axes[0].set_title("S (Slope) Distribution (512x512)", fontsize=13)
    axes[0].legend()
    ...
    axes[1].set_title("R (Roughness) Distribution (512x512)", fontsize=13)
    axes[1].legend()
    ...
    axes[2].set_title("C (Complexity) Distribution (512x512)", fontsize=13)
    axes[2].legend()
    axes[2].grid(True, alpha=0.3)

    plt.suptitle("S/R/C Feature Distribution (512x512 Resolution)", fontsize=16, y=1.02)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()

    print(f"Saved feature distribution: {output_path}")
...
def main():
    ...
    features_path = base_dir / "features_512.csv"
    visuals_dir = base_dir / "visuals_512"
    ...
    print("Reading features_512.csv...")
    ...
    print(f"\n✅ Visualization complete! Output directory: {visuals_dir}")
